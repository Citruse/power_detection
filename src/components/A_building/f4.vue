          ...
          <div class="body_1">
            <el-row class="btn_row" v-for="item in west_rooms" :key="item.id">
              <el-button
                ...
                @click="gerCurRoom(['西', 400 + item]), draws()"
                >西{{ 400 + item }}</el-button
              >
              <el-button
                class="floor_btn"
                type="primary"
                ...
                @click="gerCurRoom(['西', 401 + item]), draws()"
                >西{{ 401 + item }}</el-button
              >
            </el-row>
          </div>
          <div class="body_2">
            ...
            <div class="body_2_inner">
              <el-row class="btn_row" v-for="item in mid_rooms" :key="item.id">
                <el-button
                  ...
                  @click="gerCurRoom(['中', 400 + item]), draws()"
                  >中{{ 400 + item }}</el-button
                >
                <el-button
                  class="floor_btn"
                  type="primary"
                  ...
                  @click="gerCurRoom(['中', 401 + item]), draws()"
                  >中{{ 401 + item }}</el-button
                >
                <el-button
                  class="floor_btn"
                  type="primary"
                  ...
                  size="mini"
                  :disabled="mid_able[item+1]"
                  @click="gerCurRoom(['中', 402 + item]), draws()"
                  >中{{ 402 + item }}</el-button
                >
                <el-button
                  class="floor_btn"
                  type="primary"
                  ...
                  @click="gerCurRoom(['中', 403 + item]), draws()"
                  >中{{ 403 + item }}</el-button
                >
              </el-row>
            </div>
          </div>
          <div class="body_3">
            <el-row class="btn_row" v-for="item in east_rooms" :key="item.id">
              <el-button
                ...
                @click="gerCurRoom(['东', 400 + item]), draws()"
                >东{{ 400 + item }}</el-button
              >
              <el-button
                class="floor_btn"
                type="primary"
                ...
                @click="gerCurRoom(['东', 401 + item]), draws()"
                >东{{ 401 + item }}</el-button
              >
            </el-row>
          </div>
        </dv-border-box-3>
...
export default {
  data() {
    return {
      ...
      power_data: {},
      // 存储可用与否的列表
      east_able:[true,true,true,true,true,true,true,true,true,true,true,true,true,true],
      mid_able:[true,true,true,true,true,true,true,true],
      west_able: [true,true,true,true,true,true,true,true,true,true,true,true,true,true],
    };
  },
  ...
  methods: {
    ...
    drawHistory() {
      ...
      myChart.setOption({
        ...
        series: [
          {
            name: "日",
            data: [12, 30, 19, 21, 35, 42, 85],
            type: "bar",
          },
          {
            name: "周",
            data: [12, 22, 19, 21, 35, 42, 39],
            type: "bar",
          },
          {
            name: "月",
            data: [30, 28, 19, 21, 35, 42, 35],
            type: "bar",
          },
        ],
      });
    ...
    async getStatus() {
      ...
      for(var i=0;i<datas.length;i++){
        var num = datas[i].replace(/[^0-9]/ig,"");
        num -= 401;
